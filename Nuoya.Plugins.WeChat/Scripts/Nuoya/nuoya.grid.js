(function ($) {
    $.Nuoya.grid = function (options) {
        ...
        var _getCheckNote = function (fieldName) {
            fieldName = $.isEmptyObject(fieldName) ? "UNID" : fieldName;
        ...
        var _del = function (delOptions) {
            ...
            if (ids.length == 0) {
                ...
                $.Nuoya.deleteAction(delOptions.ajaxUrl, { unids: ids.join(",") }, delOptions.callback);
            }
        }
        ...
        var _batchAction = function (options) {
            ...
            if (ids.length == 0) {
                ...
                if (options.message != null) {
                    $.Nuoya.alertAction(options.message, options.ajaxUrl, $.extend({ unids: ids.join(",") }, options.params), options.callback);
                }
                else {
                    $.Nuoya.action(options.ajaxUrl, $.extend({ unids: ids.join(",") }, options.params), options.callback);
                }
            }
        }
