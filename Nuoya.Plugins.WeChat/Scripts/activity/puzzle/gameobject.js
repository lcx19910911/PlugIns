...
var puzzleObj = null;  //对象
var moveStartPointX = 0; //移动开始位置的x点
var moveStartPointY = 0;//移动开始位置的y点
var moveEndPointX = 0; //移动结束位置的x点
var moveEndPointY = 0; //移动结束位置的y点
var cellIndexMove = 0;//移动位置
...
puzzleGame.prototype = {
	...
	cellExchange:function(from,to){
        ...
        //判断移动的是否超过一块图片
		var moveValue = (rowTo - rowFrom) * (rowTo - rowFrom) + (colTo - colFrom) * (colTo - colFrom);
		if (moveValue > 1)
		{
		    self.cellReturn(from);
		    return;
		}
...
function  movePosition(event) {
    ...
    puzzleObj.imgCells.eq(cellIndexMove).css({
        'z-index': '40',
        'left': (touchStart.pageX - moveStartPointX - puzzleObj.imgArea.offset().left) + 'px',
        'top': (touchStart.pageY - moveStartPointY - puzzleObj.imgArea.offset().top) + 'px'
    });
    moveEndPointX = touchStart.pageX;
    moveEndPointY = touchStart.pageY;
}

//移动结束的
function endPosition() {
    //被交换的碎片下标
    var cellIndex_2 = puzzleObj.cellChangeIndex((moveEndPointX - puzzleObj.imgArea.offset().left), (moveEndPointY - puzzleObj.imgArea.offset().top), cellIndexMove);

    puzzleObj.imgCells.eq(cellIndexMove).removeClass('hover');
    if (cellIndexMove == cellIndex_2) {
        puzzleObj.cellReturn(cellIndexMove);
    } else {
        puzzleObj.cellExchange(cellIndexMove, cellIndex_2);
    }
